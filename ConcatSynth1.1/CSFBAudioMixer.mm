...
{
    static CSFBAudioMixer *sharedAudioManager;
    
    @synchronized(self)
    {
        if (!sharedAudioManager) {
            sharedAudioManager = [[CSFBAudioMixer alloc] init];
        }
...
- (id)init
{
    ...
    checkStatus(status);
    NSLog(@"checking where data is coming from 2");
    ...
    status = AudioUnitSetProperty(ioUnit,
                                  ...
                                  sizeof(audioFormat));
    checkStatus(status);
    NSLog(@"checking where data is coming from 3");
    ...
    NSLog(@"checking where data is coming from 4");
    ...
    //  [self loadFromDLSOrSoundFontName:withPatch:0];
...
-(OSStatus) loadFromDLSOrSoundFontName: (NSString *)name withPatch: (int)presetNumber {
    ...
    NSArray *pathComponents = [NSArray arrayWithObjects:
                      ...
                      name,
                      nil];
    bankURL = [NSURL fileURLWithPathComponents:pathComponents];
   ...
   /* OSStatus result = noErr;
    ...
    return result;*/
    OSStatus result = noErr;
    CFArrayRef urlArrayRef = (__bridge CFArrayRef)bankURL;
    return result = AudioUnitSetProperty(samplerUnit,
...
static OSStatus playbackCallback (void *inRefCon, AudioUnitRenderActionFlags  *ioActionFlags, const AudioTimeStamp *inTimeStamp, UInt32 inBusNumber, UInt32 inNumberFrames, AudioBufferList *ioData) {
    ...
    CSFBAudioMixer *THIS = (__bridge CSFBAudioMixer*) inRefCon;
